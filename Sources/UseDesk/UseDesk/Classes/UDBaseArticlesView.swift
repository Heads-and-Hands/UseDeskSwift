...
class UDBaseArticlesView: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    private func firstState() {
        ...
        tableView.register(UINib(nibName: "UDBaseArticleViewCell", bundle: .module), forCellReuseIdentifier: "UDBaseArticleViewCell")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        usedesk?.addViewsArticle(articleID: id, count: 1, connectionStatus: { _ in
        }, errorStatus: { _, _ in})
        indexOpenedArticle = indexPath.row
        usedesk?.getArticle(articleID: id, connectionStatus: { [weak self] success, article in
            guard let wSelf = self else {return}
            if success {
